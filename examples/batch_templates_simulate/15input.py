...
change 10input.py and track changes by git. Here we have a second script to
ease running this example.
"""
...
if __name__ == "__main__":
    ...
    df = ps.prep_batch(params, simulate=True)
    print(df)

    assert len(df) == 8
    assert df._run_seq.values[6:].tolist() == [1, 1]
    assert df._pset_seq.values[6:].tolist() == [6, 7]

    df1 = ps.df_read("calc/database.pk")
    assert df[:6].equals(df1)
